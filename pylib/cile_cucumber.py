...
"""A Code Intelligence Language Engine for the Cucumber language.
...
Module Usage:
    from cile_cucumber import scan
    mtime = os.stat("bar.cucumber")[stat.ST_MTIME]
    content = open("bar.cucumber", "r").read()
    scan(content, "bar.cucumber", mtime=mtime)
"""
...
class CucumberCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.cucumber")
...
def scan_buf(buf, mtime=None, lang="Cucumber"):
    """Scan the given CucumberBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {CucumberBuffer} is the Cucumber buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Cucumber CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Cucumber content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Cucumber <example-Cucumber-file>
    ...
    # Dev Note:
    # This is where you process the Cucumber content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Cucumber-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Cucumber content, you may
    #   want to use it.
